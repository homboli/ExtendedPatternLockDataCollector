...
public class LockPatternView extends View{
    ...
    private boolean mFadePattern = false;
    ...
    public PatternLockSession patternLockSession;
    public PatternLockSession patternLockSessionContinuous;
    private VelocityTracker mVelocityTracker = null;
    public PatternLockSensorManager patternLockSensorManager;
    ...
    public void setFinger(String mFinger) {
    ...
    public LockPatternView(Context context, AttributeSet attrs) {
        super(context, attrs);
        long patternLockSessionStartTime = Calendar.getInstance().getTimeInMillis();
        patternLockSensorManager = new PatternLockSensorManager(context);
        patternLockSession = new PatternLockSession(context);
        patternLockSession.setUserId(mUserId);
        patternLockSession.setComment(mComment);
        patternLockSession.setFinger(mFinger);
        patternLockSession.setPosition(mPosition);
        patternLockSession.setSessionStartTime(patternLockSessionStartTime);
        patternLockSessionContinuous = new PatternLockSession(context);
        patternLockSessionContinuous.setUserId(mUserId);
        patternLockSessionContinuous.setComment(mComment);
        patternLockSessionContinuous.setFinger(mFinger);
        patternLockSessionContinuous.setPosition(mPosition);
        patternLockSessionContinuous.setSessionStartTime(patternLockSessionStartTime);

        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.LockPatternView,
                R.attr.lockPatternStyle, R.style.LockPatternView);
    ...
    private void resetPattern() {
        ...
        long patternLockSessionStartTime = Calendar.getInstance().getTimeInMillis();
        patternLockSession = new PatternLockSession(getContext());
        patternLockSession.setUserId(mUserId);
        patternLockSession.setComment(mComment);
        patternLockSession.setFinger(mFinger);
        patternLockSession.setPosition(mPosition);
        patternLockSession.setSessionStartTime(patternLockSessionStartTime);
        patternLockSessionContinuous = new PatternLockSession(getContext());
        patternLockSessionContinuous.setUserId(mUserId);
        patternLockSessionContinuous.setComment(mComment);
        patternLockSessionContinuous.setFinger(mFinger);
        patternLockSessionContinuous.setPosition(mPosition);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (mUseLockPatternDrawable) {
            mNotSelectedDrawable.setBounds(getPaddingLeft(), getPaddingTop(), width, height);
            mSelectedDrawable.setBounds(getPaddingLeft(), getPaddingTop(), width, height);
        }
    }
     ...
     */
    private Cell detectAndAddHit(float x, float y, MotionEvent event) {
        ...
        if (cell != null) {
            ...
            if (fillInGapCell != null &&
                ...
                PatternPointData patternPointData = new PatternPointData(event, true);
                patternPointData.setxVelocity(xVelocity);
                patternPointData.setyVelocity(yVelocity);
                patternPointData.setRow(cell.row);
                patternPointData.setColumn(cell.column);
                patternPointData.setSensorData(new PatternLockSensorData(patternLockSensorManager.getPatternLockSensorData()));
                patternLockSession.addPatternPointData(patternPointData);
            }
            ...
            xVelocity = mVelocityTracker.getXVelocity(pointerId);
            yVelocity = mVelocityTracker.getYVelocity(pointerId);
            PatternPointData patternPointData = new PatternPointData(event, false);
            patternPointData.setxVelocity(xVelocity);
            patternPointData.setyVelocity(yVelocity);
            patternPointData.setRow(cell.row);
            patternPointData.setColumn(cell.column);
    ...
    private void handleActionMove(MotionEvent event) {
        ...
        patternPointData.setyVelocity(yVelocity);
        patternPointData.setSensorData(new PatternLockSensorData(patternLockSensorManager.getPatternLockSensorData()));
        patternLockSessionContinuous.addPatternPointData(patternPointData);
        ...
        for (int i = 0; i < historySize + 1; i++) {
            ...
            Cell hitCell = detectAndAddHit(x, y, event);
    ...
    private void sendAccessEvent(int resId) {
        this.announceForAccessibility(getContext().getString(resId));
    }
    ...
    private void handleActionDown(MotionEvent event) {
        ...
        final Cell hitCell = detectAndAddHit(x, y, event);
        if (hitCell != null) {
            setPatternInProgress(true);
            mPatternDisplayMode = DisplayMode.Correct;
            notifyPatternStarted();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            for (int j = 0; j < 3; j++) {
                ...
                if (mUseLockPatternDrawable) {
                    patternLockSessionContinuous.setCirclePosition(i, j, centerX, centerY + translationY);
                    patternLockSession.setCirclePosition(i, j, centerX, centerY + translationY);
    ...
    private void drawCellDrawable(Canvas canvas, int i, int j, float radius,
        ...
        Rect dst = new Rect(
                (int) (getPaddingLeft() + j * mSquareWidth),
                (int) (getPaddingTop() + i * mSquareHeight),
                (int) (getPaddingLeft() + (j + 1) * mSquareWidth),
                (int) (getPaddingTop() + (i + 1) * mSquareHeight));
    ...
    private final class PatternExploreByTouchHelper extends ExploreByTouchHelper {
        ...
        private CharSequence getTextForVirtualView(int virtualViewId) {
            ...
            return res.getString(R.string.lockscreen_access_pattern_cell_added_verbose, virtualViewId);
        }
